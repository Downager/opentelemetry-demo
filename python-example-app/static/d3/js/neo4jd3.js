            ...
            function Neo4jD3(_selector, _options) {
                ...
                function appendOutlineToNode(node) {
                    return node.append('circle')
                        .attr('class', 'outline')
                        .attr('r', function (d) {
                            if (Number(d.properties['d3_node_font_size']) === 1) {
                                return '0';
                            } else
                            {
                            return d.properties['d3_r'];
                            }
                        })
                ...
                function appendTextToNode(node) {
                    return node.append('text')
                        ...
                        .attr('font-size', function (d) {
                            var text_size = Number(d.properties['d3_text_size']);
                            return 4 * text_size + 'px';
                        })
                        .attr('pointer-events', 'none')
                        .attr('text-anchor', 'middle')
                        .attr('y', function (d) {
                            // console.log(d.properties['d3_node_font_size']);
                            if (Number(d.properties['d3_node_font_size']) == 6){
                                return '16px';
                            }
                            else if (Number(d.properties['d3_node_font_size']) >= 2) {
                                return '6px';
                            }else if (Number(d.properties['d3_node_font_size']) === 1) {
                                return '1px'
                            }
                                return parseInt(18+Math.round(d.properties['d3_text_y'] )) + 'px'

                            // return Number(d.properties['d3_text_size'])===4  || Number(d.properties['d3_text_size'])===3 ? '0px': (parseInt(18+Math.round(d.properties['d3_text_y'] )) + 'px');
                            // return '0px';
                        })
                            .attr('font-weight', function (d) {
                            // console.log(d.properties['d3_node_font_size']);
                            if (Number(d.properties['d3_node_font_size']) >=4) {
                                return 'bold';
                            }else {
                                return 'normal'
                            }
                ...
                // change the releation color here
...
//            color = options.colors[Math.min(numClasses, options.colors.length - 1)];
                    ...
                    var simulation = d3.forceSimulation()
                        ...
                        .force('collide', d3.forceCollide().radius(function (d) {
                            if (Number(d.properties['d3_node_font_size']) === 1) {
                                ...
                                return (Math.round(d.properties['d3_collide']) *2.5 )* (options.minCollision * 0.25 )*Number(d.properties['d3_node_font_size'])+Number(d.properties['frequency']/600);
                            }
                ...
                */
                ...
                function zoomFit(transitionDuration) {
                    ...
                    var node = svg.node();
                    var parentElement = node ? node.parentElement : null;
                    var parentElementOfParent = parentElement ? parentElement.parentElement : null;

                    if (parentElementOfParent) {
                        var fullWidth = parentElementOfParent.clientWidth;
...
//        smoothTransform(svgTranslate, svgScale);
                        ...
                        console.log('The parent element of parent is null');
                    }
